...
def _pick_predict_fn(model: PyFuncModel) -> Callable[[pd.DataFrame], np.ndarray | pd.Series]:
    """
    ...
    unwrapped_model = model.unwrap_python_model() if hasattr(model, "unwrap_python_model") else model
...
class ModelLoader:
    ...
    def load_model_bundle(self, key: str, model_name: str) -> ModelBundle:
        ...
        """
        logger.info("Starting model loading process", model_key=key, model_name=model_name)
        ...
        logger.info("Selected model version", version=model_info.version, stage=model_info.stage)
        ...
        logger.info("Warming up model with sample payload...", payload_cols=list(warmup_payload.columns))
        start_time = time.monotonic()
        predict_fn(warmup_payload)
        end_time = time.monotonic()
        warmup_latency = (end_time - start_time)

        MODEL_WARMUP_LATENCY_SECONDS.labels(model_name=model_name, model_version=model_info.version).observe(warmup_latency)
